...
try {
    ...
    echo "=== ASSIGN 'admin.panel' PERMISSION TO SUPPORT AGENT ROLE ===\n\n";
    ...
    echo "Found Support Agent role ID: " . $role['id'] . "\n\n";

    // Get admin.panel permission ID
    $stmtPerm = $pdo->prepare("SELECT id FROM permissions WHERE name = 'admin.panel'");
    $stmtPerm->execute();
    $perm = $stmtPerm->fetch(PDO::FETCH_ASSOC);

    if (!$perm) {
        echo "❌ admin.panel permission tidak ditemukan!\n";
        exit;
    }

    echo "Found admin.panel permission ID: " . $perm['id'] . "\n\n";
    ...
    if ($existing) {
        ...
        echo "✅ Permission 'admin.panel' berhasil di-assign ke Support Agent role!\n";
    }

    // Verify
    echo "\n" . str_repeat("=", 60) . "\n";
    echo "Verifying Support Agent permissions sekarang:\n";
